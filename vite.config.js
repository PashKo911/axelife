	...
	if (!isWpBlocks) {
		return {
			define: {
				...
				aliases: aliases,
				__GSAP_DEVTOOLS__: !isProduction && (templateConfig.gsap?.devTools ?? false),
			},
			resolve: {
				alias: {
					vue: 'vue/dist/vue.esm-bundler.js',
					...aliases,
					...(templateConfig.gsap?.enable === false
						? { '@js/gsap/config.js': path.resolve(process.cwd(), 'src/js/gsap/gsap-stub.js') }
						: {}),
				},
			},
			...
			build: {
				...
				rollupOptions: {
					input: isWp
						...
						: globSync('./src/*.html', { ignore: [`./src/${templateConfig.devcomponents.filename}`] }),
					...
					output: [
						{
							...
							assetFileNames: (asset) => {
								let getPath =
									asset.originalFileNames[0] && asset.names && asset.names.length > 0
										? asset.originalFileNames[0].replace(`/${asset.names[0]}`, '')
										: ''
								let extType = asset.names && asset.names.length > 0 ? asset.names[0].split('.').pop() : ''
								if (/css/.test(extType)) {
									return templateConfig.js.bundle.enable || templateConfig.server.buildforlocal
		...
		return {
			...
			build: {
				...
				rollupOptions: {
					input: [
						...globSync('./src/components/wordpress/fls-theme/components/blocks/**/*.js', {
							ignore: ['**/dist/**'],
						}),
